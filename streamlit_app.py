...
if st.session_state.processed_df is not None:
    ...
    if not filtered_df.empty:
        ...
        # What to Minimize: Campaigns with ROAS < 3
        risky_campaigns = roas_by_campaign[roas_by_campaign['ROAS'] < 3]
        if not risky_campaigns.empty:
            st.markdown("**Minimize or Review Campaigns with Low ROAS (<3):**")
            ...
            st.markdown("#### Drill Down: Targeting Value-wise ROAS (< 3) for a Campaign")
            selected_low_roas_campaign = st.selectbox(
                "Select a Campaign to drill down",
                options=risky_campaigns['Campaign Name'].tolist(),
                key="low_roas_drilldown"
            )

            campaign_detail_df = filtered_df[filtered_df['Campaign Name'] == selected_low_roas_campaign]
            targeting_value_roas_df = campaign_detail_df.groupby('Targeting Value').agg({
                'Total Sales': 'sum',
                'Estimated Budget Consumed': 'sum'
            }).reset_index()
            targeting_value_roas_df = targeting_value_roas_df[targeting_value_roas_df['Estimated Budget Consumed'] > 0]
            targeting_value_roas_df['ROAS'] = targeting_value_roas_df['Total Sales'] / targeting_value_roas_df['Estimated Budget Consumed']
            targeting_value_roas_df = targeting_value_roas_df[targeting_value_roas_df['ROAS'] < 3].sort_values(by='ROAS')

            if not targeting_value_roas_df.empty:
                st.markdown(f"**Targeting Values with ROAS < 3 for `{selected_low_roas_campaign}`:**")
                st.dataframe(targeting_value_roas_df[['Targeting Value', 'ROAS', 'Estimated Budget Consumed']])
            else:
                st.info(f"No targeting values with ROAS < 3 found for `{selected_low_roas_campaign}`.")
        else:
        ...
        risky_targeting = risky_targeting[risky_targeting['ROAS'] < 3]
        if not risky_targeting.empty:
            st.markdown("**Minimize Targeting Types with Low ROAS (<3):**")
    ...
    if not filtered_df.empty:
        ...
        low_roas_campaigns = roas_campaign_df[roas_campaign_df['ROAS'] < 3].sort_values(by='ROAS')
