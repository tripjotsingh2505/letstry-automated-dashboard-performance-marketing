...
def build_roas_summary_table(base_df, group_col):
    """Group base_df by group_col and return a table with columns:
    group_col, Most Viewed Position, Impressions, Total Sales,
    Estimated Budget Consumed, CPM, ROAS — sorted by Estimated Budget
    Consumed in descending order (highest to lowest)."""
    agg_dict = {'Total Sales': 'sum', 'Estimated Budget Consumed': 'sum'}
    if 'Impressions' in base_df.columns:
        agg_dict['Impressions'] = 'sum'

    grouped = base_df.groupby(group_col).agg(agg_dict).reset_index()

    if 'Impressions' not in grouped.columns:
        grouped['Impressions'] = 0

    if 'Most Viewed Position' in base_df.columns:
        mvp_series = base_df.groupby(group_col)['Most Viewed Position'].agg(
            lambda x: x.mode().iloc[0] if not x.mode().empty else 'N/A'
        ).reset_index()
        grouped = grouped.merge(mvp_series, on=group_col, how='left')
    else:
        grouped['Most Viewed Position'] = 'N/A'

    grouped = grouped[grouped['Estimated Budget Consumed'] > 0]
    grouped['ROAS'] = grouped['Total Sales'] / grouped['Estimated Budget Consumed']
    grouped['CPM'] = grouped.apply(
        lambda r: (r['Estimated Budget Consumed'] / r['Impressions'] * 1000) if r['Impressions'] > 0 else 0,
        axis=1
    )

    display_cols = [group_col, 'Most Viewed Position', 'Impressions', 'Total Sales',
                     'Estimated Budget Consumed', 'CPM', 'ROAS']
    grouped = grouped[display_cols].sort_values(by='Estimated Budget Consumed', ascending=False)
    return grouped
...
if st.session_state.processed_df is not None:
    ...
    if not filtered_df.empty:
        ...
        total_roas = total_sales / total_budget_spend if total_budget_spend > 0 else 0
        ...
        col7.metric("Total ROAS Across Campaigns", f"{total_roas:.2f}")
        ...
        # What to Minimize: Campaigns with ROAS < 3 and high spend
        risky_campaigns = roas_by_campaign[(roas_by_campaign['ROAS'] < 3) & (roas_by_campaign['Estimated Budget Consumed'] > 1000)]
        risky_campaigns = risky_campaigns.sort_values(by='Estimated Budget Consumed', ascending=False)
        if not risky_campaigns.empty:
            st.markdown("**Minimize or Review Campaigns with Low ROAS (<3) and High Spend:**")
            combined_risky_records = []
            for _, row in risky_campaigns.iterrows():
                # Targeting Values with ROAS < 3 for this campaign
                campaign_detail_df = filtered_df[filtered_df['Campaign Name'] == row['Campaign Name']]
                targeting_value_roas_df = build_roas_summary_table(campaign_detail_df, 'Targeting Value')
                targeting_value_roas_df = targeting_value_roas_df[targeting_value_roas_df['ROAS'] < 3]

                with st.expander(f"`{row['Campaign Name']}` — ROAS: {row['ROAS']:.2f}, Budget Spent: {row['Estimated Budget Consumed']:,.2f}"):
                    if not targeting_value_roas_df.empty:
                        st.dataframe(targeting_value_roas_df)
                    else:
                        st.info("No targeting values with ROAS < 3 for this campaign.")

                if not targeting_value_roas_df.empty:
                    record = targeting_value_roas_df.copy()
                    record.insert(0, 'Campaign Name', row['Campaign Name'])
                    record.insert(1, 'Campaign ROAS', row['ROAS'])
                    record.insert(2, 'Campaign Budget Spent', row['Estimated Budget Consumed'])
                    combined_risky_records.append(record)

            if combined_risky_records:
                combined_risky_df = pd.concat(combined_risky_records, ignore_index=True)
                csv_data = combined_risky_df.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="📥 Download Risky Campaigns & Keywords (CSV)",
                    data=csv_data,
                    file_name="risky_campaigns_keywords.csv",
                    mime="text/csv"
                )
        else:
            st.markdown("No campaigns identified as risky based on current filters.")

        # What to Minimize: Targeting Types with ROAS < 3
        ...
        risky_targeting = risky_targeting[(risky_targeting['ROAS'] < 3) & (risky_targeting['Estimated Budget Consumed'] > 1000)]
        risky_targeting = risky_targeting.sort_values(by='Estimated Budget Consumed', ascending=False)
        if not risky_targeting.empty:
            st.markdown("**Minimize Targeting Types with Low ROAS (<3) and High Spend:**")
    ...
    if not filtered_df.empty:
        roas_campaign_df = build_roas_summary_table(filtered_df, 'Campaign Name')
        top_roas_campaigns = roas_campaign_df.sort_values(by='ROAS', ascending=False).head(5)
        top_roas_campaigns = top_roas_campaigns.sort_values(by='Estimated Budget Consumed', ascending=False)

        st.subheader("Top 5 Campaigns by ROAS")
        st.dataframe(top_roas_campaigns)
    else:
    ...
    if not filtered_df.empty:
        roas_campaign_df = build_roas_summary_table(filtered_df, 'Campaign Name')
        low_roas_campaigns = roas_campaign_df[roas_campaign_df['ROAS'] < 3]

        if not low_roas_campaigns.empty:
            st.subheader("Campaigns with ROAS less than 3")
            st.dataframe(low_roas_campaigns)
        else:
            st.info("No campaigns with ROAS less than 3 found.")
    else:
